...
penjualanRouter.post("/", async (req, res) => {
  ...
  const totalDiscRp = payload.items.reduce(
    (sum, item) => sum + item.qty * (item.disc_rp || 0),
    0,
  );
  const totalHargaBarang = payload.items.reduce(
    (sum, item) => sum + item.qty * (item.harga_jual - (item.disc_rp || 0)),
    0,
  );
